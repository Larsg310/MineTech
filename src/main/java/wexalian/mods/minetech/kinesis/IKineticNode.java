...
public interface IKineticNode extends INBTSerializable<NBTTagCompound>
{
    ...
    {
        return MineTech.kineticNodeProvider.apply(host);
    }
    ...
    static void findShaft(World world, BlockPos pos, EnumFacing side, float ratio, ObjFloatConsumer<IKineticNode> neighbors, BiPredicate<World, BlockPos> posValidator)
    {
        pos = pos.offset(side);
    ...
    static void findGear(World world, BlockPos pos, EnumFacing face, EnumFacing side, float ratio, ObjFloatConsumer<IKineticNode> neighbors, BiPredicate<World, BlockPos> posValidator)
    {
        pos = pos.offset(side);
        if (posValidator.test(world, pos))
        {
