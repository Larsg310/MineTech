...
public class BlockGear extends Block implements ITileEntityProvider
{
    
    private static final AxisAlignedBB[] BOXES = new AxisAlignedBB[]{new AxisAlignedBB(0, 0, 0, 1, 2 / 16F, 1), new AxisAlignedBB(0, 14 / 16F, 0, 1, 1, 1), new AxisAlignedBB(0, 0, 0, 1, 1, 2 / 16F), new AxisAlignedBB(0, 0, 14 / 16F, 1, 1, 1), new AxisAlignedBB(0, 0, 0, 2 / 16F, 1, 1), new AxisAlignedBB(14 / 16F, 0, 0, 1, 1, 1)};
    ...
    public void onEntityCollidedWithBlock(World world, BlockPos pos, IBlockState state, Entity entity)
    {
        ...
        AxisAlignedBB bb = Objects.requireNonNull(getCollisionBoundingBox(state, world, pos)).offset(pos).expand(0.0, 0.1, 0.0);
        
        if (!entity.getEntityBoundingBox().intersects(bb)) return;
        
        float velocity = Optional.ofNullable(world.getTileEntity(pos)).filter(it -> it instanceof TileGear).map(it -> ((TileGear) it).getVelocity()).orElse(0f);
    ...
    @Nonnull
    @Override
    public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer, @Nullable EnumHand hand)
    {
